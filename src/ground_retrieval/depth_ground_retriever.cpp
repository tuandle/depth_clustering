...
#include "./depth_ground_retriever.h"
...
void DepthGroundRetriever::OnNewObjectReceived(const Cloud& cloud,
                                             const int sender_id) {
  ...
  auto ground_image = ZeroOutGroundBFS(depth_image, smoothed_image,
                                          _ground_retrieve_angle, _window_size);
  fprintf(stderr, "INFO: Ground retrieved in %lu us\n", total_timer.measure());
  cloud_copy.projection_ptr()->depth_image() = ground_image;
  this->ShareDataWithAllClients(cloud_copy);
  _counter++;
}

Mat DepthGroundRetriever::ZeroOutGround(const cv::Mat& image,
...
Mat DepthGroundRetriever::ZeroOutGroundBFS(const cv::Mat& image,
                                         const cv::Mat& angle_image,
...
Mat DepthGroundRetriever::RepairDepth(const Mat& no_ground_image, int step,
...
Mat DepthGroundRetriever::RepairDepth(const Mat& depth_image) {
...
Mat DepthGroundRetriever::CreateAngleImage(const Mat& depth_image) {
...
Mat DepthGroundRetriever::GetSavitskyGolayKernel(int window_size) const {
  if (window_size % 2 == 0) {
...
Mat DepthGroundRetriever::GetUniformKernel(int window_size, int type) const {
  if (window_size % 2 == 0) {
    throw std::logic_error("only odd window size allowed");
  }
...
Mat DepthGroundRetriever::ApplySavitskyGolaySmoothing(const Mat& image,
                                                    int window_size) {
...
Radians DepthGroundRetriever::GetLineAngle(const Mat& depth_image, int col,
